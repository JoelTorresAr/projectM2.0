  ...
  <v-app id="app">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat class="mt-2">
          ...
          <v-dialog v-model="dialogForm" persistent max-width="600px">
            <template v-slot:activator="{ on }" v-if="can('subsidiaries.create')">
      ...
      <template v-slot:item.actions="{ item }">
        <v-btn color="primary" fab x-small dark @click="editItem(item)" v-if="can('subsidiaries.edit')">
          <v-icon>mdi-pencil</v-icon>
        </v-btn>
        <v-btn color="red" fab x-small dark @click="deleteItem(item)" v-if="can('subsidiaries.destroy')">
...
export default {
  ...
  created() {
    this.$store.dispatch("can", "subsidiaries.index");
    this.initialize();
  },
